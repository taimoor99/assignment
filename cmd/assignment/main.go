...
import (
	...
	"github.com/taimoor99/assignment/app/delivery/rest"
...
func main() {
	...
	goc, err := mongo.GetSession(ctx, os.Getenv("DATABASE_NAME"), os.Getenv("MONGODB_URL"))
	if err != nil {
	...
	repo := mongo.NewMessageRepository(goc)
	model := models.NewMessageModel(repo)
	ctrl := rest.NewMessage(model)
